...
from ..objects.Ton.Ton import Ton
from ..objects.Ton.TonWallet import TonWallet
...
class TonController:
    def __init__(self):
        ...
        # self.tonapi = self.ton_wallet.ton.tonapi
        # self.tonapi.websocket_url = os.getenv("TON_WEBSOCKET_URL")
        self.logger = Logger(name="ton_controller").get_logger()
        self.logger.info(f"Base wallet: {self.ton_wallet.ton.base_wallet}")
        self.logger.info(f"Jetton address: {self.ton_wallet.ton.jetton_address}")

    # async def websocket_subscribe(self):
    #     await self.tonapi.websocket.subscribe_to_transactions([self.ton_wallet.ton.base_wallet], self.get_event)
    #
    ...
    async def create_transaction(self):
        await self.ton_wallet.create_ton_transaction(self.ton_wallet.ton.test_wallet, 3)

    async def wallet_balance(self):
        await self.ton_wallet.wallet_balance()

    # async def base_wallet_info(self):
    #     wallet_info = await self.tonapi.accounts.get_info(self.ton_wallet.ton.base_wallet)
    #     self.logger.info(
    ...
    #     jetton = await self.tonapi.jettons.get_info(self.ton_wallet.ton.jetton_address)
    #     self.logger.info(
    #         f"Jetttons:\n"
